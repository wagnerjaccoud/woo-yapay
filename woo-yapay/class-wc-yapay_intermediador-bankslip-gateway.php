...
class WC_Yapay_Intermediador_Bankslip_Gateway extends WC_Payment_Gateway {
    ...
    public function init_form_fields() {
		...
		//adiciona opção para inserir dias de vencimento do boleto bancário **INICIO
		...
		//adiciona opção para inserir dias de vencimento do boleto bancário **FIM
            'environment' => array(
    ...
    */
	...
	// CALCULAR DATA DE VENCIMENTO E CASO SEJA FINAL DE SEMANA O VENCIMENTO SERÁ NA SEGUNDA FEIRA **INICIO
		...
		if($dia_semana == 6){
			$params["payment[billet_date_expiration]"]  = date( 'd/m/Y', $timestamp + ( 2 * 86400 ) );
			$data_vencimento_interno  = date( 'Y-m-d', $timestamp + ( 2 * 86400 ) );
		}else{
			if($dia_semana == 7){
				$params["payment[billet_date_expiration]"] = date( 'd/m/Y', $timestamp + ( 1 * 86400 ) );
				$data_vencimento_interno  = date( 'Y-m-d', $timestamp + ( 1 * 86400 ) );
			}else{
				$params["payment[billet_date_expiration]"] = $data_vencimento;
				$data_vencimento_interno  = date( 'Y-m-d', $data_vencimento );
			}
		}
        update_post_meta( $order->id, 'data_vencimento_boleto', $data_vencimento_interno ); // META DADO PARA TRATATIVAS INTERNAS
	// CALCULAR DATA DE VENCIMENTO E CASO SEJA FINAL DE SEMANA O VENCIMENTO SERÁ NA SEGUNDA FEIRA **FIM    
	...
	// META DADOS PARA TRATAMENTOS INTERNOS NO WOOCOMMERCE **INICIO
	update_post_meta( $order->id, 'linha_digitavel', $tcTransaction->typeful_line );
	update_post_meta( $order->id, 'loja5_woo_bradesco_api_link_boleto', $tcTransaction->url_payment );
	// META DADOS PARA TRATAMENTOS INTERNOS NO WOOCOMMERCE **FIM
